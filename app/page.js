...
export default function Page(){
 ...
 const finish=()=>{if(!done.includes(current.id)){setDone([...done,current.id]);setStars(stars+5)}setGallery([{...current,date:new Date().toLocaleDateString("de-DE")},...gallery.filter(x=>x.id!==current.id)]);setScreen("celebrate")};
 return <main>
  ...
  {screen==="celebrate"&&<section className="reward celebrationScreen"><Logo/><div className="rewardgrid"><Lion/><div><h1>Super gemacht!</h1><p>Du hast ein wunderschönes Bild gemalt! 💗</p><Thumb it={current} done/></div></div><div className="prizes"><div>⭐<b>+5 Sterne</b></div><div>🏅<b>Kreativ Künstler</b></div><div>🎁<b>+1 Überraschung</b></div></div><div className="actions"><button onClick={()=>setScreen("gallery")}>Zur Galerie</button><button className="finish" onClick={()=>open(items[(items.findIndex(x=>x.id===current.id)+1)%items.length])}>Nächstes Bild</button></div></section>}
  ...
  {screen==="reward"&&<section className="rewardsHub">
   ...
   <div className="levelCard">
    ...
    <div className="levelFoot"><span>🎨 {done.length} fertig</span><span>Noch {done.length%5===0&&done.length>0?5:5-(done.length%5)} bis zum nächsten Level ✨</span></div>
   </div>

   <div className="rewardsSectionHead"><div><span className="eyebrow">Sammlung</span><h2>Deine Abzeichen</h2></div><span>{Math.min(6,1+Math.floor(done.length/2))}/6</span></div>
   ...
   <div className="surpriseGrid">
    <article className={`surpriseCard ${stars>=50?"ready":""}`}><span>🎁</span><div><b>Kleine Schatzkiste</b><small>50 Sterne</small></div><em>{stars>=50?"Bereit!":"Noch "+Math.max(0,50-stars)+" ⭐"}</em></article>
    ...
    <article className={`surpriseCard ${stars>=200?"ready":""}`}><span>👑</span><div><b>Goldene Überraschung</b><small>200 Sterne</small></div><em>{stars>=200?"Bereit!":"Noch "+Math.max(0,200-stars)+" ⭐"}</em></article>
   </div>
