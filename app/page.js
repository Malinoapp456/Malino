...
export default function Page(){
 ...
 const achievementBadges=[
  ...
  {id:"gallery",icon:"🖼️",title:"Galerie-Star",desc:"10 Werke in der Galerie",ok:gallery.length>=10},
  ...
  {screen==="start"&&<section className="home premiumHome">
   <div className="premiumHero">
    ...
    <div className="premiumHeroMascot" aria-hidden="true">
     <div className="heroMascotBubble">🦁</div>
     <span>✨</span><span>⭐</span><span>🎨</span>
    </div>
   </div>

   <div className="homeQuickStats">
    <button onClick={()=>setScreen("rewards")}><span>⭐</span><b>{stars}</b><small>Sterne</small></button>
    <button onClick={()=>setScreen("rewards")}><span>🏅</span><b>{unlockedBadgeCount}/{achievementBadges.length}</b><small>Abzeichen</small></button>
    <button onClick={()=>setScreen("gallery")}><span>🖼️</span><b>{gallery.length}</b><small>Galerie</small></button>
    <button onClick={()=>open(dailyItem)}><span>🔥</span><b>{dailyStreak}</b><small>Tage Serie</small></button>
   </div>

   <div className="homeSectionHeader">
    <div><span className="eyebrow">Heute</span><h2>Dein Tages-Abenteuer</h2></div>
    <button onClick={()=>open(dailyItem)}>Öffnen ›</button>
   </div>
   ...
   <div className="todayAdventureGrid">
    <div className={`daily dailyV2 dailyChallengeCard premiumDailyCard ${dailyClaimed?"dailyDone":""}`}>
     ...
     <div className="dailyChallengeInfo"><b>{dailyItem.name}</b><small>{dailyClaimed?"Heute schon geschafft. Morgen wartet ein neues Bild!":"Male das heutige Bild fertig und sichere dir 10 Bonus-Sterne."}</small></div>
     <button onClick={()=>open(dailyItem)}>{dailyClaimed?"Noch einmal malen":"Challenge starten!"}</button>
    </div>
    ...
    <button className="streakCard streakCardButton premiumStreakCard" onClick={()=>open(dailyItem)} aria-label="Tages-Challenge öffnen">
     ...
     <div className="streakText"><b>{dailyStreak} Tage Serie</b><small>{dailyStreak===0?"Starte heute deine erste Serie!":dailyClaimed?"Heute gesichert ✓ – Challenge erneut öffnen":"Schließe die Tages-Challenge ab, um die Serie zu halten."}</small></div>
   ...
   <div className="continueCard">
    <div className="continueThumb"><Thumb it={continueItem} done={done.includes(continueItem.id)}/></div>
    <div className="continueCopy">
     <span>{recentItem?"Dein letztes Bild":"Für dich ausgewählt"}</span>
     <h3>{continueItem.name}</h3>
     <p>{recentItem?"Mach dort weiter, wo du zuletzt aufgehört hast.":"Starte mit einem Bild, das heute gut zu deinem Malino-Abenteuer passt."}</p>
     <button onClick={()=>open(continueItem)}>🖌️ Jetzt malen</button>
    </div>
   </div>
   ...
   <div className="homeFeatureGrid">
    ...
    <button className="featureCard featureGallery" onClick={()=>setScreen("gallery")}>
     <span>🖼️</span><div><b>Galerie</b><small>Deine fertigen Kunstwerke</small></div><em>›</em>
    </button>
