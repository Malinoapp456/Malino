...
const RasterPainter=forwardRef(function RasterPainter({src,color,tool},ref){
 ...
 const fillAt=e=>{
  ...
  while(top){const p=stack[--top];if(barrier[p]||!matches(p))continue;const i=p*4;d[i]=fill[0];d[i+1]=fill[1];d[i+2]=fill[2];d[i+3]=205;const px=p%w,py=(p/w)|0;
 ...
 return <div className="rasterPainter"><img id="malino-paint-image" src={src} alt="Malbild" draggable="false" onLoad={drawBase}/><canvas ref={baseRef} className="baseMap" aria-hidden="true"/><canvas ref={canvasRef} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up}/></div>
});
export default function Page(){
 ...
 const open=it=>{setCurrent(it);setFills({});setHistory([]);setTool("fill");setScreen("paint")};
