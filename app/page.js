...
export default function Page(){
 ...
 const headerStarCount=Number(profileData[activeProfileId]?.stars ?? stars ?? 0);
 ...
 return <main>
  <header><button>🇩🇪 Deutsch⌄</button><Logo/><div><span>⭐ {headerStarCount}</span><button onClick={()=>setScreen("parent")}>👤 {activeProfile.name}</button></div></header>
