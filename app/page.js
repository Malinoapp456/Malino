...
export default function Page(){
 ...
 return <main>
  ...
  {screen==="start"&&<section>
   ...
   <div className="panel"><h2>Wähle eine Kategorie</h2><div className="cats">{cats.slice(1).map(c=><button key={c} onClick={()=>{setCat(c);setScreen("library")}}><span>{categoryMeta[c].emoji}</span><b>{categoryMeta[c].label}</b><small>{categoryCount(c)} {categoryCount(c)===1?"Bild":"Bilder"}</small></button>)}</div>
  ...
  {screen==="library"&&<section className="libraryPage">
   ...
   <div className="libraryCategories">
    {cats.slice(1).map(c=>{const meta=categoryMeta[c];const count=categoryCount(c);return <button key={c} className={`libraryCategory ${meta.tone} ${cat===c?"isSelected":""}`} onClick={()=>setCat(c)}>
